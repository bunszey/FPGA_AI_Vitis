...
void float_to_fixed(float input[n_inputs], ap_fixed<32,8> output[n_inputs]) {
...
void fixed_to_float(ap_fixed<32,8> input[n_inputs], float output[n_inputs]) {
	for(int i = 0; i < n_inputs; i++){
		output[i] = input[i];
	}
}
...
void hwmm_layer1(ap_fixed<32,8> input[n_inputs], const ap_fixed<32,8> weights[n_inputs][n_layer1], ap_fixed<32,8> output[1][n_layer1]) {
...
void hwmm_layer2(ap_fixed<32,8> input[1][n_layer1], const ap_fixed<32,8> weights[n_layer1][n_layer2], ap_fixed<32,8> output[1][n_layer2]) {
...
void hwmm_layer3(ap_fixed<32,8> input[1][n_layer2], const ap_fixed<32,8> weights[n_layer2][n_layer3], ap_fixed<32,8> output[1][n_layer3]) {
...
void hw_act_layer1(ap_fixed<32,8> input[1][n_layer1], ap_fixed<32,8> output[1][n_layer1]){
...
void hw_act_layer2(ap_fixed<32,8> input[1][n_layer2], ap_fixed<32,8> output[1][n_layer2]){
...
void hw_act_layer3(ap_fixed<32,8> input[1][n_layer3], ap_fixed<32,8> &pred){
	ap_fixed<32,8> max_idx = -1;
	ap_fixed<32,8> max_val = -126;
	loop1: for (int i = 0; i < n_layer3; i++){
#pragma HLS UNROLL
		//std::cout << input[0][i]  << std::endl;
		if (input[0][i] > max_val){
			max_idx = (ap_fixed<32,8>)i;
			max_val = input[0][i];
		}
	}
	pred = max_idx;
...
/* Connect NN Layers */
//ap_fixed<32,8> nn_inference(float input_img[n_inputs]){
...
//#pragma HLS ARRAY_PARTITION dim=1 type=complete variable=input_img
	...
	ap_fixed<32,8> fp_input_img[n_inputs] = {1.0};
	float_to_fixed(input_img, fp_input_img);

	ap_fixed<32,8> temp_output[1][n_layer1] = {1};
	ap_fixed<32,8> temp_output2[1][n_layer2] = {1};
	ap_fixed<32,8> temp_output3[1][n_layer3] = {1};
	ap_fixed<32,8> prediction = -1;
